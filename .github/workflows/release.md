...
description: |
  ...
  release notes from git history since the last release. The PR
  requires human review and approval before merge. When merged,
...
3. **Determine the next version**: Read the git tags to find the latest
   release tag (tags matching `v*`). If no tags exist, use `v0.1.0`.
   Otherwise, increment the minor version (e.g., `v0.2.0` → `v0.3.0`).
   This project uses `v0.x.0` minor releases per its release policy.
